...
if($input_password == NULL){
    $_SESSION['sign_up_message'] = 'Invalid password. Please enter a valid password and try again.';
...
//FILTER_VALIDATE_EMAIL will return false if not valid email, or NULL if empty
    ...
    $_SESSION['sign_up_message'] = 'Invalid Email Address. Please enter a valid email address and try again.';
    $_SESSION['show_message'] = TRUE;
    header('Location: sign_up.php');
    exit;
}
...
{
    $_SESSION['sign_up_message'] = 'Invalid Data. Please make sure all required fields have valid data and and try again.';
    $_SESSION['show_message'] = TRUE;
    header('Location: sign_up.php');
    exit;
}
//if country is US and state field is empty, display error message
    ...
    $_SESSION['sign_up_message'] = 'Please enter the state since the country is US (United Stated) and try again.';
    $_SESSION['show_message'] = TRUE;
    header('Location: sign_up.php');
    exit;
}
//if everything else is valid, then we must first check that this email does not already have an account
else{
    ...
    //if query was successful, the user already has an account, because email is unique in this table, 
    //so display a message asking the user to log in instead
    if(count($result) > 0){
        $_SESSION['sign_in_message'] = "You already have an account! Please sign in instead.";
        $_SESSION['show_message'] = TRUE;
        $_SESSION['sign_up_message'] = 'You already have an account! Please sign in instead.';
        header('Location: ../index.php');
        exit;
    }
    //otherwise if no previous records were found, reate a new address record, then get its ID
    //then encrypt the password and create a new user record with the address ID that we just created
        ...
        //only if no same address was found, create new address and get it to have the addressID
        if(count($result) == 0)
        {
            ...
            //if the state has been chosen, make sure we look for it too so bind its value to a parameter
            if($US_state != NULL){
                $statement->bindValue(':stateCode', $US_state);
            }        
            $statement->bindValue(':countryCode', $country_code);
        ...
        //make sure we got something back by counting results and ensuring it is one address result
        ...
        if($row_count == 1){
            ...
            foreach ($result as $address){
                ...
                //if query was successful, the user has been successfully added, so log the user in
                if(count($result) > 0){
                    $_SESSION['sign_up_message'] = "";
                    $_SESSION['show_message'] = FALSE;
                    $_SESSION['loggedin'] = TRUE;
                    $_SESSION['loggedin_as'] = $first_name;
                    $_SESSION['loggedin_userid'] = $result['userID'];
                    header('Location: ../index.php');
                    exit;
                }
                //if no users found then query was not successful
                else{
                    $_SESSION['sign_up_message'] = 'There was an error while creating your account. Please try again later.'. $encrypted_password;
                    $_SESSION['show_message'] = TRUE;
                    header('Location: sign_up.php');
                    exit;
                }
        ...
        //if we got no addresses back or more than 1 when getting addressID, display an error
            ...
            $_SESSION['sign_up_message'] = 'There was an error while saving your address. Please try again later.';
            $_SESSION['show_message'] = TRUE;
            header('Location: sign_up.php');
            exit;
        }
